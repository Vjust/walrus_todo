...
export default class StorageCommand extends BaseCommand {
  ...
  async showStorageDetails(walrusStorage: WalrusStorage) {
    ...
    try {
      ...
      for (const item of response.data) {
        ...
        this.log(
          `${chalk.bold('Remaining Time:')} ${remainingEpochs > 0 ? `${remainingEpochs} epochs` : 'Expired'}`
        );
      }
      ...
      this.log(
        `${chalk.bold('Active Storage Objects:')} ${activeCount} of ${response.data.length}`
      );
    } catch (error) {
      if (error instanceof CLIError) {
        throw error;
  ...
  async analyzeStorageEfficiency(walrusStorage: WalrusStorage) {
    ...
    try {
      ...
      this.log(
        `Current WAL balance: ${chalk.green(walBalance.totalBalance)} WAL`
      );
      ...
      const analyzer = (walrusStorage as any)
      ...
      const analyzeAndDisplay = async (size: number, description: string) => {
        ...
        if (analysis.analysisResult.bestMatch) {
          ...
          this.log(
            `Remaining after operation: ${(match.remaining - size).toLocaleString()} bytes`
          );
          this.log(
            `WAL tokens saved by reusing: ${chalk.green(analysis.costComparison.reuseExistingSavings.toString())} WAL`
          );
          this.log(
            `Percentage saved: ${chalk.green(analysis.costComparison.reuseExistingPercentSaved.toString())}%`
          );
        } else {
          this.log(
            `New storage cost estimate: ${chalk.yellow(analysis.costComparison.newStorageCost.toString())} WAL`
          );
        }
      };
      ...
      if (overallAnalysis.totalStorage === 0) {
        this.log(
          chalk.yellow(
            'You have no storage allocations. Consider creating storage when storing todos.'
          )
        );
      } else if (overallAnalysis.activeStorageCount === 0) {
        this.log(
          chalk.red(
            'All your storage allocations have expired. Create new storage for your todos.'
          )
        );
      } else if (overallAnalysis.availableStorage < 1024 * 1024) {
        // Less than 1MB available
        this.log(
          chalk.yellow(
            'Your available storage is limited. Consider allocating more storage.'
          )
        );
      } else if (
        overallAnalysis.availableStorage > 10 * 1024 * 1024 &&
        overallAnalysis.usedStorage < overallAnalysis.totalStorage * 0.1
      ) {
        this.log(
          chalk.blue(
            'You have significant unused storage. Consider using it efficiently before allocating more.'
          )
        );
      } else {
        this.log(
          chalk.green('Your storage allocation appears to be efficient.')
        );
      }
      ...
      this.log(
        '1. Group multiple todos in a TodoList to save on storage costs'
      );
      this.log(
        '2. Use existing storage when possible instead of allocating new storage'
      );
      this.log(
        '3. Consider using the `--analyze` flag before storing large amounts of data'
      );
      this.log(
        '4. Periodically check your storage allocation with `walrus-todo storage`'
      );
    } catch (error) {
      if (error instanceof CLIError) {
        throw error;
