...
export default class AiPermissions extends BaseCommand {
  static description =
    'Control AI provider access levels and operation permissions with blockchain verification';
  ...
  static flags = {
    ...
    operation: Flags.string({
      ...
      required: true,
      options: ['list', 'check', 'grant', 'revoke', 'register'],
    }),
  ...
  private async initializePermissionManager() {
    ...
    if (!flags.registry || !flags.packageId) {
      this.warn(
        chalk.yellow(
          '⚠️  Blockchain integration not configured. Some features will be limited.'
        )
      );
      this.warn(
        chalk.dim(
          'Use --registry and --packageId to enable blockchain verification.'
        )
      );
  ...
  private async listPermissions(
    ...
    try {
      if (flags.provider) {
        ...
        const permissionLevel = await permissionManager.getPermissionLevel(
          flags.provider
        );
        ...
        if (credentials.length === 0) {
          ...
          this.log(
            chalk.dim(
              `To add a credential, run: ${chalk.cyan('walrus_todo ai:credentials add --provider [name] --apiKey [key]')}`
            )
          );
          return;
        }
        ...
        if (flags.format === 'json') {
          ...
          for (const credential of credentials) {
            ...
            if (operations.length === 0) {
              ...
              this.log(
                `  Operations:       ${operations.slice(0, 5).join(', ')} ${chalk.dim(`+ ${operations.length - 5} more`)}`
              );

              this.log(
                chalk.dim(
                  `  To see all operations, run: ${chalk.cyan(`walrus_todo ai:permissions list --provider ${credential.providerName}`)}`
                )
              );
            }
          }
  ...
  private async checkPermission(
    ...
    if (!flags.provider) {
      this.error(
        'Provider name is required for checking permissions. Use --provider flag.'
      );
    }

    if (!flags.aiOperation) {
      this.error(
        'AI operation name is required for checking permissions. Use --aiOperation flag.'
      );
    }
          ...
          try {
            ...
            if (verificationResult.allowed) {
              ...
              if (verificationResult.verificationId) {
                this.log(
                  `Verification ID:  ${chalk.dim(verificationResult.verificationId)}`
                );
              }
            } else {
            ...
            this.warn(
              chalk.yellow(`⚠️ Blockchain verification failed: ${error}`)
            );
          }
        }
      }
  ...
  private async grantPermission(
    ...
    if (!flags.provider) {
      this.error(
        'Provider name is required for granting permissions. Use --provider flag.'
      );
    }

    if (!flags.permission) {
      this.error(
        'Permission level is required for granting permissions. Use --permission flag.'
      );
    }
    ...
    try {
      ...
      if (success) {
        this.log(
          chalk.green(
            `✓ Permission level for '${flags.provider}' updated to ${flags.permission}`
          )
        );
        ...
        if (flags.aiOperation) {
          ...
          this.log(
            `Operation '${flags.aiOperation}' is now ${hasPermission ? chalk.green('allowed') : chalk.red('denied')}`
          );
        ...
        this.error(
          `Failed to update permission level for provider '${flags.provider}'`
        );
      }
    } catch (error) {
  ...
  private async revokePermission(
    ...
    if (!flags.provider) {
      this.error(
        'Provider name is required for revoking permissions. Use --provider flag.'
      );
    }
    ...
    try {
      ...
      this.log(
        chalk.yellow(
          `⚠️  Are you sure you want to revoke permissions for '${flags.provider}'?`
        )
      );
      ...
      if (success) {
        this.log(
          chalk.green(`✓ Permissions for '${flags.provider}' have been revoked`)
        );
        ...
        if (flags.verify) {
          ...
          try {
            ...
            if (verificationResult.allowed) {
              this.log(
                chalk.red(
                  '✗ Permission verification failed - should not be allowed after revocation'
                )
              );
            } else {
              this.log(
                chalk.green('✓ Permission change verified on blockchain')
              );
            }
          } catch (_error) {
            ...
            this.log(
              chalk.green(
                '✓ Permission change verified (permission denied as expected)'
              )
            );
          }
        }
        ...
        this.error(
          `Failed to revoke permissions for provider '${flags.provider}'`
        );
      }
    } catch (error) {
  ...
  private async registerOperation(
    ...
    if (!permissionManager) {
      this.error(
        'Blockchain integration is required for registering operations. Please provide --registry and --packageId flags.'
      );
    }

    if (!flags.aiOperation) {
      this.error(
        'AI operation name is required for registration. Use --aiOperation flag.'
      );
    }

    if (!flags.permission) {
      this.error(
        'Permission level is required for operation registration. Use --permission flag.'
      );
    }
    ...
    try {
      ...
      this.log(
        chalk.green(
          `✓ Operation '${flags.aiOperation}' registered with permission level ${flags.permission}`
        )
      );
