...
export const StorageLocationSchema = z.enum(['local', 'walrus', 'sui'], {
  errorMap: () => ({ message: 'Storage location must be one of: local, walrus, sui' })
});
...
export const TodoSchema = z.object({
  ...
  title: z.string()
    ...
    .refine(title => title.trim().length > 0, 'Todo title cannot be only whitespace'),
  ...
  errorMap: (issue, ctx) => {
    if (issue.code === z.ZodIssueCode.invalid_type) {
      return { message: `Expected ${issue.expected} but received ${issue.received} for field "${issue.path.join('.')}"` };
    }
 ...
 */
export const TodoListArraySchema = z.array(TodoListSchema)
 ...
 */
export function validateTodoSafe(todo: unknown): { success: true; data: TodoSchemaType } | { success: false; error: string } {
  try {
    ...
    if (error instanceof z.ZodError) {
      ...
      return { success: false, error: `Todo validation failed: ${formattedErrors}` };
    }
 ...
 */
export function validateTodoListSafe(todoList: unknown): { success: true; data: TodoListSchemaType } | { success: false; error: string } {
  try {
    ...
    if (error instanceof z.ZodError) {
      ...
      return { success: false, error: `TodoList validation failed: ${formattedErrors}` };
    }
    return { success: false, error: 'TodoList validation failed: Unknown error' };
  }
}
