import { SpinnerManager, ErrorHandler, FlagValidator, RetryManager, Logger, Formatter } from '../cli-helpers';
...
describe('CLI Helpers', () => {
  ...
  describe('ErrorHandler', () => {
    it('should handle CLIError and rethrow', () => {
      ...
      expect(() => ErrorHandler.handle(cliError, 'test context')).toThrow(CLIError);
    });

    it('should wrap non-CLIError in CLIError', () => {
      const normalError = new Error('Normal error');
      
      expect(() => ErrorHandler.handle(normalError, 'test context')).toThrow(CLIError);
    });
    ...
    it('should exit process with error', () => {
      ...
      expect(mockConsoleError).toHaveBeenCalledWith(expect.stringContaining('Exit message'));
  ...
  describe('FlagValidator', () => {
    it('should validate positive numbers', () => {
      ...
      expect(() => FlagValidator.validatePositiveNumber('-123', 'test')).toThrow(CLIError);
      expect(() => FlagValidator.validatePositiveNumber('abc', 'test')).toThrow(CLIError);
      expect(() => FlagValidator.validatePositiveNumber('0', 'test')).toThrow(CLIError);
    });

    it('should validate non-empty strings', () => {
      expect(FlagValidator.validateNonEmpty('test', 'field')).toBe('test');
      expect(FlagValidator.validateNonEmpty('  test  ', 'field')).toBe('test');
      expect(() => FlagValidator.validateNonEmpty('', 'field')).toThrow(CLIError);
      expect(() => FlagValidator.validateNonEmpty('   ', 'field')).toThrow(CLIError);
    });

    it('should validate enums', () => {
      const validValues = ['high', 'medium', 'low'] as string[];
      expect(FlagValidator.validateEnum('high', validValues, 'priority')).toBe('high');
      expect(() => FlagValidator.validateEnum('invalid', validValues, 'priority')).toThrow(CLIError);
    });

    it('should validate paths', () => {
      expect(FlagValidator.validatePath('/valid/path', 'path')).toBe('/valid/path');
  ...
  describe('Logger', () => {
    it('should log success messages', () => {
      ...
      expect(mockConsoleLog).toHaveBeenCalledWith(expect.stringContaining('Success message'));
    });
    ...
    it('should log warning messages', () => {
      ...
      expect(mockConsoleWarn).toHaveBeenCalledWith(expect.stringContaining('Warning message'));
    });
    ...
    it('should log step messages', () => {
      ...
      expect(mockConsoleLog).toHaveBeenCalledWith(expect.stringContaining('[1/5]'));
      expect(mockConsoleLog).toHaveBeenCalledWith(expect.stringContaining('Step message'));
    });
  });
