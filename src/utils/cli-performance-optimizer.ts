...
export class CLIPerformanceOptimizer {
  ...
  private generateEnvironmentHash(): string {
    ...
    return createHash('md5').update(JSON.stringify(envData)).digest('hex');
  }
  ...
  private async analyzeAndCacheModules(): Promise<void> {
    ...
    const frequentModules = this.getFrequentlyUsedModules(metrics);
  ...
  private generateConfigHash(): string {
    try {
      ...
      return createHash('md5').update(packageJson).digest('hex');
    } catch (error) {
      return 'unknown';
    }
  }
  ...
  private getFrequentlyUsedModules(metrics: CLIMetrics[]): string[] {
    ...
    return Object.entries(moduleUsage)
      .sort(([, a], [, b]) => b - a)
      .slice(0, 10)
      .map(([module]) => module);
  }
...
export function measurePerformance(target: any, propertyName: string, descriptor: PropertyDescriptor) {
  ...
  descriptor.value = async function (...args: any[]) {
    ...
    try {
      ...
      optimizer.endCommand(propertyName);
    }
  };
