...
export class LegacyWalrusImageStorageAdapter {
   ...
   */
  async retrieveImage(blobId: string): Promise<{ imageData: Uint8Array; metadata: Record<string, string> }> {
 ...
 */
export function createLegacyWalrusStorage(useMockMode = false): LegacyWalrusStorageAdapter {
 ...
 */
export function createLegacyWalrusImageStorage(useMockMode = false): LegacyWalrusImageStorageAdapter {
  // Get address from environment or config
  const address = process.env.SUI_ADDRESS || '0x0000000000000';
