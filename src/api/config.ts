  ...
  constructor(options: ApiConfigOptions = {}) {
    ...
    this.cors = {
      origins: options.cors?.origins || 
        (process.env.API_CORS_ORIGINS?.split(',') || ['http://localhost:3000', 'http://localhost:3001'])
    };

    this.auth = {
      required: options.auth?.required ?? (process.env.API_AUTH_REQUIRED === 'true'),
      apiKeys: options.auth?.apiKeys || 
        (process.env.API_KEYS?.split(',') || [])
    };

    this.logging = {
      enabled: options.logging?.enabled ?? (process.env.API_LOGGING_ENABLED !== 'false'),
