...
async function installGlobally() {
  ...
  const buildResult = spawnSync(
    'node',
    ['scripts/run-build.js', '--transpile-only'],
    {
      stdio: 'inherit',
      shell: true,
    }
  ...
  if (!isWindows) {
    ...
    if (needsSudo) {
      print(
        'yellow',
        "You don't have write permission to global bin directory."
      );
      print(
        'yellow',
        `Using sudo to install globally with ${packageManager}...`
      );
  ...
  if (verifyResult.status === 0) {
    ...
    print(
      'red',
      "Installation verification failed. 'waltodo' command not found."
    );
    if (!isWindows) {
      print(
        'yellow',
        'Try running with sudo: sudo node scripts/install-global.js'
      );
    } else {
