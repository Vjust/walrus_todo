...
const importPatterns = [
  {
    ...
    usage: {
      'createWalrusStorage\\((.*)\\)':
        "createStorage('todo', undefined, { useMockMode: $1 }) as TodoStorage",
  ...
  },
  {
    ...
    usage: {
      'createWalrusImageStorage\\((.*)\\)':
        "createStorage('image', undefined, { suiClient: $1 }) as ImageStorage",
      'new WalrusImageStorage\\((.*)\\)':
        "createStorage('image', undefined, { suiClient: $1 }) as ImageStorage",
    },
  },
  {
    ...
    usage: {
      'new SuiNftStorage\\(([^,]*),([^,]*),(.*)\\)':
        "createStorage('nft', undefined, { suiClient: $1, signer: $2, packageConfig: $3 }) as NFTStorage",
    },
  },
];
...
function generateMigrationSuggestions(files) {
  ...
  logger.info(
    'For each file, update the imports and usage patterns as shown above.'
  );
  logger.info(
    'Remember to adjust the configuration parameters based on the specific needs of each file.'
  );
}
