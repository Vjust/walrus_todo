...
const Logger_1 = require("../apps/cli/src/utils/Logger");
const logger = new Logger_1.Logger('unified-build');
/**
...
const options = {
    ...
    binPermissionFix: args.includes('--fix-permissions') ||
...
function cleanDist() {
    ...
    if (fs.existsSync(distDir)) {
        try {
            ...
            logger.error(`${colors.red}✗ Failed to clean dist directory:${colors.reset}`, error instanceof Error ? error : new Error(String(error)));
            process.exit(1);
        }
    }
...
function fixBinPermissions() {
    ...
    try {
            ...
            binFiles.forEach(file => {
                ...
                if (fs.statSync(filePath).isFile()) {
                    try {
                        ...
                        logger.warn(`${colors.yellow}⚠ Could not change permissions for ${filePath}:${colors.reset}`, { error: error instanceof Error ? error.message : String(error) });
                    }
                }
            });
        ...
        logger.error(`${colors.red}✗ Failed to fix bin directory permissions:${colors.reset}`, error instanceof Error ? error : new Error(String(error)));
    }
}
/**
...
function touchManifest() {
    ...
    try {
        ...
        logger.error(`${colors.red}✗ Failed to create/touch manifest file:${colors.reset}`, error instanceof Error ? error : new Error(String(error)));
    }
}
/**
...
function runTypeScriptCompiler() {
    ...
    try {
        ...
        logger.error(`${colors.red}✗ Failed to run TypeScript compiler:${colors.reset}`, error instanceof Error ? error : new Error(String(error)));
        if (!options.skipTypeCheck) {
            process.exit(1);
        }
    }
    ...
    try {
        ...
        allFiles.forEach(fileName => {
            try {
                ...
                logger.error(`${colors.red}Error processing ${fileName}:${colors.reset}`, error instanceof Error ? error : new Error(String(error)));
                errors++;
            }
        });
        logger.info(`${colors.green}✓ Build completed with ${filesProcessed} files successfully transpiled and ${errors} errors.${colors.reset}`);
        if (errors > 0) {
            logger.warn(`${colors.yellow}⚠ There were ${errors} errors during transpilation.${colors.reset}`);
        }
    }
    catch (error) {
        logger.error(`${colors.red}✗ Failed to run transpile-only build:${colors.reset}`, error instanceof Error ? error : new Error(String(error)));
        process.exit(1);
    }
}
/**
