...
async function launchDefaultNft() {
  try {
    ...
    const suiClient = new SuiClient({ url: (NETWORK_URLS as Record<string, string>)[CURRENT_NETWORK] });
    ...
    if (
      ...
      TODO_NFT_CONFIG.MODULE_ADDRESS.length < 10
    ) {
    ...
    process.stdout.write(
      `✓ Using NFT module at address: ${TODO_NFT_CONFIG.MODULE_ADDRESS}\n`
    );
    ...
    if (todoList && todoList.todos.length > 0) {
      ...
      process.stdout.write(
        `✓ Using existing Todo: "${todoItem.title}" (ID: ${todoItem.id})\n`
      );
    } else {
      ...
      const newTodoData = {
        ...
        description:
          'This todo was created to demonstrate NFT integration with Walrus storage',
        completed: false,
      ...
      process.stdout.write(
        `✓ Created new Todo: "${todoItem.title}" (ID: ${todoItem.id})\n`
      );
    }
    ...
    const nftStorage = new SuiNftStorage(suiClient, keypair, {
      address: addressInfo,
...
launchDefaultNft().then(result => {
  if (result) {
    ...
    process.stdout.write('==================================\n');
    process.stdout.write(
      'To view your NFT, you can use the Sui Explorer or a compatible wallet.\n'
    );
  } else {
