...
export default class RetrieveCommand extends BaseCommand {
  ...
  static flags = {
    ...
    list: Flags.string({
      char: 'l',
      description: 'Save to this todo list (or source list for positional syntax)',
      default: 'default',
    }),
    mock: Flags.boolean({
  ...
  private async retrieveAllFromList(listName: string): Promise<void> {
    ...
    this.stopSpinner(true, `Found ${todos.length} todo(s) in list "${listName}"`);
    ...
    todos.forEach((todo, index) => {
      ...
      this.log(`  ${status} ${chalk.bold(todo.title)} ${chalk.dim(`[${priority}]`)}`);
      
      if (todo.walrusBlobId) {
        this.log(`    ${chalk.dim('Walrus:')} ${chalk.cyan(todo.walrusBlobId)}`);
      }
      if (todo.nftObjectId) {
    ...
    try {
        ...
        switch (identifierType) {
          case 'list':
            ...
            if (args.todoTitle) {
              ...
              this.stopSpinner(true, `Will retrieve "${todoTitle}" from list "${sourceList}"`);
            } else {
              // Retrieve all todos from list
              retrieveAll = true;
              this.stopSpinner(true, `Will retrieve all todos from list "${sourceList}"`);
            }
            break;
      ...
      if (todoTitle) {
        ...
        if (!blobId && !objectId) {
          ...
          this.log(
            chalk.yellow('⚠️'),
            'You must specify what to retrieve'
          );
          this.log(chalk.dim('\nPositional syntax (recommended):'));
          this.log(chalk.dim(`  ${this.config.bin} retrieve mylist                    # Retrieve all from list`));
          this.log(chalk.dim(`  ${this.config.bin} retrieve mylist "Task Title"      # Retrieve specific todo`));
          this.log(chalk.dim(`  ${this.config.bin} retrieve <blob-id>               # Retrieve by blob ID`));
          this.log(chalk.dim(`  ${this.config.bin} retrieve <object-id>             # Retrieve by object ID`));
          
          this.log(chalk.dim('\nLegacy flag syntax:'));
          this.log(chalk.dim(`  ${this.config.bin} retrieve --todo "My Task" --list ${flags.list}`));
          this.log(chalk.dim(`  ${this.config.bin} retrieve --blob-id <id> --list ${flags.list}`));
          this.log(chalk.dim(`  ${this.config.bin} retrieve --object-id <id> --list ${flags.list}`));
          ...
          if (mockMode) {
            ...
            this.log(chalk.dim(`  ${this.config.bin} retrieve mock-object-456`));
          }
      ...
      try {
        ...
        if (blobId) {
          ...
          try {
            ...
            const targetList = args.identifier && !flags.todo ? 'default' : flags.list;
  ...
  private async runInBackground(args: any, flags: any): Promise<void> {
    ...
    const job = jobManager.createJob('retrieve', 
    ...
    try {
      ...
      const operationId = await this.queueBackgroundRetrieval(args, flags, job.id, timeoutMs);
   ...
   */
  private async waitForBackgroundOperation(operationId: string, jobId: string): Promise<void> {
    if (!this.backgroundOps) {
      throw new CLIError('Background operations manager not initialized', 'BACKGROUND_NOT_INITIALIZED');
    }
