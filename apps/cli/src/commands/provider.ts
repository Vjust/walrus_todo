...
export default class Provider extends BaseCommand {
  ...
  private async providerInfo(providerIdentifier: string, format: string) {
    this.log(
      chalk.bold(`Fetching provider information for ${providerIdentifier}...`)
    );
    ...
    try {
      ...
      this.log(
        `Status:             ${provider.isActive ? chalk.green('Active') : chalk.red('Inactive')}`
      );
  ...
  private async registerProvider() {
    ...
    try {
      this.log(
        chalk.yellow(
          'This command requires additional implementation for interactive provider registration.'
        )
      );
