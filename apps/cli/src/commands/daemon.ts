...
export default class Daemon extends BaseCommand {
  static description =
    'Start the WalTodo sync daemon for real-time synchronization';
  ...
  async run(): Promise<void> {
    ...
    if (!wallet) {
      this.error(
        'Wallet address is required. Use --wallet flag or configure a default wallet.'
      );
    }
  ...
  private async runDetached(
    ...
    const child = spawn(
      ...
      [
        ...
        '--sync-interval',
        String(config.syncInterval! / 1000),
        '--conflict-resolution',
        config.conflictResolution!,
        ...(config.enableRealTimeSync ? [] : ['--no-real-time']),
        '--max-concurrent',
        String(config.maxConcurrentSyncs),
      ],
      {
        detached: true,
        stdio: 'ignore',
  ...
  private async showStatus(): Promise<void> {
    if (this.syncEngine) {
      ...
      if (apiStatus) {
        ...
        this.log(
          `WebSocket: ${apiStatus.websocketConnected ? '🟢 Connected' : '🔴 Disconnected'}`
        );
