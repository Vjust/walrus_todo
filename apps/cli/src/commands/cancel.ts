...
export default class CancelCommand extends BaseCommand {
  ...
  static flags = {
    ...
    command: Flags.string({
      char: 'c',
      description: 'Cancel jobs of this command type (e.g., retrieve, fetch, store)',
    }),
  ...
  async run() {
    ...
    if (!args.jobId) {
      this.error('Job ID required. Use --all to cancel all jobs or --pattern to match multiple jobs.');
      return;
    }
  ...
  private async cancelSingleJob(jobId: string, flags: any): Promise<void> {
    ...
    if (job.status === 'completed' || job.status === 'failed' || job.status === 'cancelled') {
      this.warn(`Job ${jobId} is already ${job.status} and cannot be cancelled`);
      return;
    }
    ...
    try {
      ...
      if (!flags.force) {
        ...
        this.log(chalk.yellow('⚠️  Graceful cancellation failed, attempting force cancellation...'));
      }
  ...
  private async cancelMultipleJobs(flags: any): Promise<void> {
    ...
    if (flags.pattern) {
      ...
      jobsToCancel = jobsToCancel.filter(job => 
        pattern.test(job.id) || pattern.test(job.command) || 
   ...
   */
  private async cancelRetrievalOperation(operationId: string, flags: any): Promise<void> {
    ...
    if (status.phase === 'complete') {
      this.warn(`Retrieval operation ${operationId} is already complete and cannot be cancelled`);
      return;
    }

    if (flags['dry-run']) {
      this.log(chalk.yellow(`Would cancel retrieval operation: ${operationId}`));
      return;
    }
    ...
    try {
      ...
      if (cancelled) {
        ...
        // Show final status
        const finalStatus = await backgroundDataRetriever.getRetrievalStatus(operationId);
        if (finalStatus) {
          this.log(chalk.dim(`\nFinal status: ${finalStatus.phase}`));
          if (finalStatus.processedItems && finalStatus.totalItems) {
            this.log(chalk.dim(`Items processed: ${finalStatus.processedItems}/${finalStatus.totalItems}`));
          }
        }
      } else {
        throw new CLIError('Failed to cancel retrieval operation', 'CANCEL_FAILED');
      }
