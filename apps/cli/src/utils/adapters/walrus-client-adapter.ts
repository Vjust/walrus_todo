...
import type { WalrusClient as OriginalWalrusClient } from '@mysten/walrus';
import type {
  WriteBlobOptions,
  ReadBlobOptions,
...
class OriginalWalrusClientAdapter extends BaseWalrusClientAdapter {
  ...
  async getBlobInfo(blobId: string): Promise<NormalizedBlobObject> {
    ...
    try {
      ...
      if (
        !('getBlobInfo' in this.walrusClient) ||
        typeof (this.walrusClient as unknown as Record<string, unknown>)
          .getBlobInfo !== 'function'
      ) {
        throw new WalrusClientAdapterError(
          'getBlobInfo method not available on client'
        );
      }

      const getBlobInfoFn = (
        this.walrusClient as {
  ...
  async writeBlob(
    ...
    const adaptedOptions = this.extractAdapters(
  ...
  async getBlobMetadata(
    ...
    const clientWithMetadata = this.walrusClient as unknown as {
      getBlobMetadata?: (
        options: ReadBlobOptions
      ) => Promise<Record<string, unknown>>;
    };
  ...
  async verifyPoA(params: { blobId: string }): Promise<boolean> {
    ...
    const clientWithVerifyPoA = this.walrusClient as unknown as {
  ...
  async getBlobObject(params: {
    ...
    const clientWithGetBlobObject = this.walrusClient as unknown as {
      getBlobObject?: (params: { blobId: string }) => Promise<unknown>;
    };
    if (
...
class ExtendedWalrusClientAdapter extends BaseWalrusClientAdapter {
  ...
  async getBlobSize(blobId: string): Promise<number> {
    ...
    try {
      ...
      if (
        ...
        typeof (this.walrusClient).getBlobSize === 'function'
      ) {
  ...
  reset(): void {
    ...
    try {
      ...
      if (
        ...
        typeof (this.walrusClient).reset === 'function'
      ) {
  ...
  get experimental():
    ...
    if (
      ...
      (this.walrusClient).experimental
    ) {
  ...
  async executeCertifyBlobTransaction(
    ...
    if (
      ...
      typeof (this.walrusClient).executeCertifyBlobTransaction ===
        'function'
    ) {
      const adaptedOptions = this.extractAdapters(
        options as Record<string, unknown>
      ...
      try {
        return await (this.walrusClient).executeCertifyBlobTransaction(
          adaptedOptions
        );
      } catch (error) {
        throw new WalrusClientAdapterError(
  ...
  async executeWriteBlobAttributesTransaction(
    ...
    if (
      ...
      typeof (this.walrusClient)
        .executeWriteBlobAttributesTransaction === 'function'
    ) {
      const adaptedOptions = this.extractAdapters(
        options as Record<string, unknown>
      );
  ...
  async executeCreateStorageTransaction(
    ...
    if (
      ...
      typeof (this.walrusClient).executeCreateStorageTransaction ===
        'function'
    ) {
      const adaptedOptions = this.extractAdapters(
        options as Record<string, unknown>
      ...
      try {
        return await (this.walrusClient).executeCreateStorageTransaction(
          adaptedOptions
        );
      } catch (error) {
        throw new WalrusClientAdapterError(
 ...
 */
export function createVersionSpecificAdapter(
  client: OriginalWalrusClient | WalrusClient | WalrusClientExt | unknown
): WalrusClientAdapter {
  if (!client) {
    throw new WalrusClientAdapterError(
...
export function extractAdapters<T extends Record<string, any>>(options: T): T {
  ...
  if (
    ...
    (result).transaction = extractTransaction(
...
export function normalizeWalrusBlobObject(
...
export function normalizeWriteBlobResponse(
  ...
  if (typeof response === 'string') {
    ...
    typeof (
      (response.blobObject as Record<string, unknown>).id as Record<
        string,
        unknown
      >
    ).id === 'string'
  ...
  if (typeof response === 'object' && response.blobObject) {
    blobObject = normalizeWalrusBlobObject(
      response.blobObject as Record<string, unknown>
    );
  } else {
