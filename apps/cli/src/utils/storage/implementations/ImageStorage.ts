...
export class ImageStorage extends BlobStorage {
  ...
  public async storeImage(
    ...
    try {
      ...
      logger.info(
        `Image dimensions: ${imageInfo.width}x${imageInfo.height}, Format: ${imageInfo.format}`
      );
   ...
   */
  public async retrieveImage(blobId: string): Promise<{
    imageData: Uint8Array;
    ...
    try {
      ...
      const imageInfo: ImageInfo = {
        ...
        size: content.length,
        aspectRatio:
          parseFloat(metadata.aspectRatio || '0') ||
          parseInt(metadata.width || '0', 10) /
            parseInt(metadata.height || '0', 10),
      };
  ...
  private validateImageData(imageData: Uint8Array, contentType: string): void {
    ...
    if (imageData.length > this.getConfig().maxContentSize) {
      throw new ValidationError(
        `Image size exceeds maximum allowed (${this.getConfig().maxContentSize} bytes)`,
        {
          operation: 'validate image',
    ...
    if (!this.validateImageFormat(imageData, format)) {
      throw new ValidationError(
        `Image data does not match claimed format: ${format}`,
        {
          operation: 'validate image',
          field: 'format',
          value: format,
