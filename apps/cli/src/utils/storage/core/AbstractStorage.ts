...
export abstract class AbstractStorage implements IStorage {
  ...
  protected static readonly cache: Map<
    string,
    { data: Uint8Array; metadata: Record<string, string>; expires: number }
  > = new Map();
   ...
   * Must be implemented by subclasses.
   */
  public abstract ensureStorageAllocated(
    sizeBytes: number
  ): Promise<StorageInfo>;
  ...
  protected validateConnection(operation: string): void {
    if (this.connectionState !== 'connected') {
      throw new ValidationError(
        `Storage not connected. Call connect() first.`,
        {
          operation,
          recoverable: true,
        }
