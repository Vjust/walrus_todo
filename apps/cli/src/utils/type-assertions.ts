...
export function assertBaseSigner(obj: unknown): obj is {
  ...
  const requiredMethods = ['signPersonalMessage', 'signWithIntent', 'getKeyScheme', 'toSuiAddress'];
 ...
 */
export function assertStringProperty(obj: unknown, propertyName: string): string {
 ...
 */
export function assertNumberProperty(obj: unknown, propertyName: string): number {
  if (!obj || typeof obj !== 'object' || !(propertyName in obj)) {
    throw new Error(`Object does not have property: ${propertyName}`);
  }
 ...
 */
export function assertTransactionObjectArgument(obj: unknown): obj is { kind: string } {
 ...
 */
export function normalizeSignatureResponse(signature: unknown): { signature: string; bytes?: string } {
