...
import {
  validateApiKey,
  performKeySecurityCheck,
} from '../../utils/KeyValidator';
import { CLIError } from '../../types/errors/consolidated';
import { Logger } from '../../utils/Logger';

const logger = new Logger('SecureCredentialService');
import {
  AICredentialAdapter,
  AIPermissionLevel,
...
export class SecureCredentialService {
  ...
  public async storeCredential(
    ...
    const permissionLevel =
      options.permissionLevel || AIPermissionLevel.STANDARD;
    const now = Date.now();
    ...
    return {
      provider,
      verified,
      expiresAt: metadataInfo?.expiresAt
        ? new Date(metadataInfo.expiresAt).toISOString()
        : undefined,
      createdAt: new Date(metadataInfo?.createdAt || now).toISOString(),
      rotationDue: metadataInfo?.rotationDue
        ? new Date(metadataInfo.rotationDue).toISOString()
        : undefined,
      permissionLevel,
    ...
    try {
      ...
      return {
        ...
        rotationDue: metadataInfo.rotationDue
          ...
          : undefined,
        permissionLevel:
          metadata?.permissionLevel || AIPermissionLevel.STANDARD,
