...
export class CredentialErrorRecovery {
  ...
  public safeEncrypt(
    ...
    try {
      ...
      if (opts.allowTestFallbacks && this.isTestEnvironment()) {
        return Buffer.from(
          JSON.stringify({ value: data, test: true }),
          'utf-8'
        );
      }
      ...
      if (opts.allowTestFallbacks && this.isTestEnvironment()) {
        ...
        return Buffer.from(
          JSON.stringify({ value: data, test: true, fallback: true }),
          'utf-8'
        );
      }
  ...
  public validateKeyIntegrity(
    ...
    try {
      ...
      if (key.length !== 32) {
        logger.error(
          `Key integrity check: Invalid key size ${key.length}, expected 32`
        );
        return false;
      }
  ...
  public getSafeConfigPaths(): {
    ...
    return {
      ...
      storeFile: path.join(
        configDir,
        isTest ? 'test-credentials.dat' : 'secure-credentials.dat'
      ),
    };
  }
