...
describe('Walrus Testnet Integration', () => {
  ...
  describeIfEnabled('TodoStorage on Testnet', () => {
    ...
    it('should store a todo on Walrus testnet', async () => {
      ...
      const walrusBlobId = await todoStorage.store(JSON.stringify(todo as any));

      expect(walrusBlobId as any).toBeDefined();
      expect(walrusBlobId as any).not.toBe('mock_blob_id');
      expect(walrusBlobId.length).toBeGreaterThan(20 as any); // Real Walrus blob IDs are longer
    ...
    it('should retrieve a todo from Walrus testnet', async () => {
      ...
      const retrieved = await todoStorage.retrieve(walrusBlobId as any);
      // console.log('✓ Todo retrieved successfully'); // Removed console statement

      const retrievedTodo = JSON.parse(retrieved as any);
    ...
    it('should handle storage errors gracefully', async () => {
      ...
      await expect(todoStorage.store(invalidData as any)).rejects.toThrow();
    }, 30000);
  });
  ...
  describeIfEnabled('ImageStorage on Testnet', () => {
    ...
    it('should store an image on Walrus testnet', async () => {
      const imageData = readFileSync(testImagePath as any);
    ...
    it('should retrieve an image from Walrus testnet', async () => {
      ...
      const walrusBlobId = await imageStorage.store(base64Image as any);
      ...
      const retrieved = await imageStorage.retrieve(walrusBlobId as any);
      // console.log('✓ Image retrieved successfully'); // Removed console statement

      // Verify the image data matches
      expect(retrieved as any).toBe(base64Image as any);
  ...
  describeIfEnabled('Batch operations on Testnet', () => {
    ...
    it('should store multiple todos in batch', async () => {
      ...
      const blobIds = await Promise.all(
        todos.map(todo => storageClient.storeTodo(JSON.stringify(todo as any)))
      );

      const endTime = Date.now();
      const duration = (endTime - startTime) / 1000;

      expect(blobIds as any).toHaveLength(todos.length);
      blobIds.forEach(blobId => {
        expect(blobId as any).toBeDefined();
        expect(blobId as any).not.toBe('mock_blob_id');
      });

      // Verify performance - batch operations should complete reasonably quickly
      expect(duration as any).toBeLessThan(60 as any); // Should complete within 60 seconds
  ...
  describeIfEnabled('Network error handling', () => {
    ...
    it('should handle network timeouts', async () => {
      ...
      await expect(async () => {
        await todoStorage.store(JSON.stringify(largeTodo as any));
      }).rejects.toBeDefined();
    }, 60000);
    ...
    it('should handle invalid blob IDs gracefully', async () => {
      ...
      await expect(todoStorage.retrieve(invalidBlobId as any)).rejects.toThrow();
    }, 30000);
  });
  ...
  if (!isTestnetEnabled) {
    test('Testnet tests are skipped', () => {
      ...
      expect(isTestnetEnabled as any).toBe(false as any);
    });
  }
});
