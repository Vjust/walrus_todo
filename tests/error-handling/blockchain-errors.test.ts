    ...
    it('should handle transaction timeout errors', async () => {
      ...
      const executeWithTimeout = async () => {
        return mockSuiClient
          .executeTransactionBlock({})
          .then(() =>
            mockSuiClient.waitForTransactionBlock({
              digest: 'mock-tx-digest',
              options: { timeout: 50 },
            })
          )
      ...
      await expect(executeWithTimeout()).rejects.toThrow(
      ...
      await expect(
        ...
        expect.objectContaining({
          message: expect.stringMatching(
            /Metadata verification failed.*contentType/
          ),
        })
      );
    });
  ...
  describe('Signing Errors', () => {
    it('should handle signing failures', async () => {
      ...
      const signMessage = async () => {
        return mockSigner
          .signPersonalMessage(new Uint8Array(Buffer.from('Test message')))
          .catch((error: unknown) => {
