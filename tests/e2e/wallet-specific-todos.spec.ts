...
async function mockWalletConnection(page: Page, walletAddress: string, walletName: string = 'TestWallet') {
  // Mock wallet connection by setting up the wallet context
  await page.evaluate(({ address, name }) => {
    ...
    localStorage.setItem('walletConnectionData', JSON.stringify(mockWalletData));
...
async function createTodo(page: Page, title: string, description: string = '', priority: 'low' | 'medium' | 'high' = 'medium') {
  // Fill in the todo form
  await page.fill('input[placeholder="What needs to be done?"]', title);
  
  if (description) {
    await page.fill('textarea[placeholder="Add a description (optional)"]', description);
  }
...
test.describe('Wallet-Specific Todo Functionality', () => {
  ...
  test('should show wallet connection prompt when no wallet is connected', async ({ page }) => {
    ...
    await expect(page.locator('text=Connect your wallet to create and manage your personal todos')).toBeVisible();
  ...
  test('should allow creating todos when wallet is connected', async ({ page }) => {
    // Mock wallet connection
    await mockWalletConnection(page, '0x1234567890abcdef1234567890abcdef12345678');
    await page.reload();
    await page.waitForLoadState('networkidle');
    
    // Wallet connection warning should be hidden
    await expect(page.locator('text=Connect your wallet to create and manage your personal todos')).not.toBeVisible();
    ...
    // Create a test todo
    await createTodo(page, 'Test Wallet Todo', 'This todo should be associated with wallet 1');
  ...
  test('should persist todos when reconnecting the same wallet', async ({ page }) => {
  ...
  test('should allow completing todos for specific wallet', async ({ page }) => {
  ...
  test('should show appropriate error messages for wallet operations', async ({ page }) => {
    ...
    await page.fill('input[placeholder="What needs to be done?"]', 'This should fail');
    await page.click('button[type="submit"]');
    
    // Should show error message
    await expect(page.locator('text=Please connect your wallet to add todos')).toBeVisible();
  });
