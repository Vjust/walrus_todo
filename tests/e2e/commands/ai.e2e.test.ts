...
jest.mock('../../../src/services/ai/AIProviderFactory', () => ({
  AIProviderFactory: {
    ...
    createFallbackAdapter: jest.fn().mockReturnValue({
      processWithPromptTemplate: jest.fn().mockResolvedValue({ result: 'Mock fallback response' }),
      completeStructured: jest.fn().mockResolvedValue({ result: {} })
    }),
    createProvider: jest.fn().mockImplementation(({ provider }) => ({
      processWithPromptTemplate: jest.fn().mockImplementation((prompt, input) => {
        // Mock responses based on operation type
        if (prompt.template.includes('Summarize')) {
          return Promise.resolve({ result: 'Your todos focus on financial and project work.' });
        }
        return Promise.resolve({ result: 'Mock response' });
      }),
      completeStructured: jest.fn().mockImplementation(({ prompt, metadata }) => {
        ...
        if (metadata?.operation === 'suggest') {
          return Promise.resolve({
            result: ['Review quarterly reports', 'Schedule team meeting', 'Update project timeline']
          });
        }
        if (metadata?.operation === 'analyze') {
          return Promise.resolve({
            result: {
              ...
              recommendations: ['Prioritize urgent tasks', 'Block time for focused work']
            }
          });
        }
...
describe('AI Command E2E Tests', () => {
  ...
  describe('summarize operation', () => {
    test('should generate a summary of todos', async () => {
      ...
      expect(result.stdout).toContain('Your todos focus on financial and project work.');
    ...
    test('should output JSON when flag is set', async () => {
      ...
      expect(output.summary).toBe('Your todos focus on financial and project work.');
      expect(result.exitCode).toBe(0);
    });
  });
  ...
  describe('provider flags', () => {
    test('should use specified provider', async () => {
      const result = await runCommand(['ai', 'summarize', '--provider', 'openai']);
    ...
    test('should use specified temperature', async () => {
      const result = await runCommand(['ai', 'summarize', '--temperature', '0.2']);
