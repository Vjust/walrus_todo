...
describe('E2E Performance Tests', () => {
  ...
  afterAll(async () => {
    ...
    performanceReport.summary.passing = performanceReport.results.filter(r => r.status === 'passing').length;
    performanceReport.summary.warning = performanceReport.results.filter(r => r.status === 'warning').length;
    performanceReport.summary.critical = performanceReport.results.filter(r => r.status === 'critical').length;
    performanceReport.summary.averageExecutionTime = executionTimes.reduce((a, b) => a + b, 0) / executionTimes.length;
    ...
    const summaryPath = path.join(resultsDir, `performance-summary-${Date.now()}.txt`);
...
`;
  ...
  describe('Basic Commands Performance', () => {
    it('should measure add command performance', async () => {
      ...
      expect(metric.executionTime).toBeLessThan(PERFORMANCE_THRESHOLDS.add.critical);
    });
    ...
    it('should measure list command performance', async () => {
      ...
      expect(metric.executionTime).toBeLessThan(PERFORMANCE_THRESHOLDS.list.critical);
    });
    ...
    it('should measure complete command performance', async () => {
      ...
      if (todoId) {
        ...
        expect(metric.executionTime).toBeLessThan(PERFORMANCE_THRESHOLDS.complete.critical);
      }
    });
    ...
    it('should measure delete command performance', async () => {
      ...
      if (todoId) {
        ...
        expect(metric.executionTime).toBeLessThan(PERFORMANCE_THRESHOLDS.delete.critical);
      }
    });
  });
  ...
  describe('AI Commands Performance', () => {
    it('should measure ai suggest command performance', async () => {
      ...
      expect(metric.executionTime).toBeLessThan(PERFORMANCE_THRESHOLDS.ai.critical);
    });
  ...
  describe('Storage Commands Performance', () => {
    it('should measure store command performance', async () => {
      ...
      expect(metric.executionTime).toBeLessThan(PERFORMANCE_THRESHOLDS.store.critical);
    });
    ...
    it('should measure fetch command performance', async () => {
      ...
      if (blobId) {
        ...
        expect(metric.executionTime).toBeLessThan(PERFORMANCE_THRESHOLDS.fetch.critical);
      }
    });
  });
