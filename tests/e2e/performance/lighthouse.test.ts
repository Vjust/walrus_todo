...
test.describe('Performance Testing', () => {
  ...
  test('Frontend Lighthouse audit meets performance thresholds', async ({
    page,
    context,
  }) => {
  ...
  test('CLI command response times are within acceptable limits', async () => {
    const commands = [
      ...
      {
        cmd: 'add',
        args: ['Performance Test Todo', 'Testing CLI performance'],
        maxTime: 3000,
      },
    ...
    results.forEach(result => {
      ...
      console.log(
        `${status} ${result.command}: ${result.responseTime}ms (max: ${result.threshold}ms)`
      );
    });
  ...
  test('Frontend handles bulk operations without performance degradation', async ({
    ...
    const todoTitles = Array.from(
      { length: bulkSize },
      (_, i) => `Bulk Performance Test ${i + 1} ${Date.now()}`
    );
    ...
    const memoryIncrease =
      finalMetrics.memoryUsage - baselineMetrics.memoryUsage;
