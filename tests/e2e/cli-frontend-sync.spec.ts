...
test.describe('CLI-Frontend Real-time Integration', () => {
  ...
  test('Frontend todo completion syncs to CLI within 2 seconds', async () => {
    ...
    while (attempts < 10) {
      // 10 attempts = 2 seconds
      const cliResult = await cli.executeJSON<TestTodo[]>('list', [
        '--format',
        'json',
      ]);
      cliTodos = cliResult;
    ...
    throw new Error(
      'Todo completion did not sync from frontend to CLI within 2 seconds'
    );
  });
  ...
  test('WebSocket connection maintains real-time sync', async () => {
    ...
    console.log(
      `✅ Rapid sync of ${todoTitles.length} todos completed in ${totalSyncTime}ms`
    );
  });
  ...
  test('Performance: Bulk operations maintain responsiveness', async () => {
    ...
    console.log(
      `✅ Bulk sync of ${bulkSize} todos completed in ${totalTime}ms`
    );
  ...
  test('Data consistency across system restart', async () => {
    ...
    const cliTodos = await cli.executeJSON<TestTodo[]>('list', [
      '--format',
      'json',
    ]);
