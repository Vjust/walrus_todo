...
export class FrontendHelpers {
  ...
  async waitForAppReady(timeout = 10000): Promise<void> {
    ...
    await this.page.waitForSelector('[data-testid="app-container"]', { timeout });
  ...
  async getTodoData(todoElement: Locator): Promise<TodoData> {
    ...
    const description = await todoElement.locator('[data-testid="todo-description"]').textContent() || '';
    const completed = await todoElement.getAttribute('data-completed') === 'true';
    const isBlockchain = await todoElement.getAttribute('data-blockchain') === 'true';
    const ownerAddress = await todoElement.getAttribute('data-owner') || undefined;
  ...
  async completeTodo(todoElement: Locator): Promise<void> {
    const completeButton = todoElement.locator('[data-testid="complete-todo-btn"]');
  ...
  async clearAllTodos(): Promise<void> {
    try {
      const clearAllButton = this.page.locator('[data-testid="clear-all-todos-btn"]');
      
      if (await clearAllButton.count() > 0) {
        await clearAllButton.click();
        
        // Confirm in modal if it appears
        const confirmButton = this.page.locator('[data-testid="confirm-clear-all"]');
  ...
  async connectWallet(): Promise<void> {
    const connectButton = this.page.locator('[data-testid="connect-wallet-btn"]');
    
    if (await connectButton.count() > 0) {
      await connectButton.click();
      
      // Select wallet type (assuming test wallet is available)
      const testWalletOption = this.page.locator('[data-testid="wallet-option-test"]');
      if (await testWalletOption.count() > 0) {
        await testWalletOption.click();
      }
      
      // Wait for wallet connection to complete
      await this.page.waitForSelector('[data-testid="wallet-connected"]', { timeout: 10000 });
    }
  }
   ...
   */
  async waitForErrorNotification(errorPattern: RegExp, timeout = 5000): Promise<void> {
    const errorNotification = this.page.locator('[data-testid="error-notification"]');
    
    await this.page.waitForSelector('[data-testid="error-notification"]', { timeout });
  ...
  async getTransactionHistory(): Promise<TransactionData[]> {
    ...
    const historyButton = this.page.locator('[data-testid="transaction-history-btn"]');
    
    if (await historyButton.count() > 0) {
      await historyButton.click();
      
      // Wait for history to load
      await this.page.waitForSelector('[data-testid="transaction-list"]', { timeout: 5000 });
      
      // Extract transaction data
      const transactions = await this.page.locator('[data-testid="transaction-item"]').all();
   ...
   */
  async createTodo(title: string, description: string, options: {
    priority?: string;
    tags?: string[];
    blockchain?: boolean;
  } = {}): Promise<void> {
    ...
    if (options.priority) {
      await this.page.selectOption('[data-testid="todo-priority-select"]', options.priority);
    }
    ...
    if (options.blockchain) {
      const blockchainCheckbox = this.page.locator('[data-testid="blockchain-todo-checkbox"]');
