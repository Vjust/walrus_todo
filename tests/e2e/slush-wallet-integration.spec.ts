...
test.describe('Slush Wallet Integration', () => {
  ...
  test('should maintain Slush wallet connection across page navigation', async ({ page }) => {
...
test.describe('Slush Wallet Todo Operations', () => {
  test('should enable blockchain features when Slush wallet connected', async ({ page }) => {
    ...
    const blockchainElement = await page.getByText(/blockchain|NFT|store on chain/i, { exact: false }).first();
    const hasBlockchainUI = await blockchainElement.isVisible().catch(() => false);
