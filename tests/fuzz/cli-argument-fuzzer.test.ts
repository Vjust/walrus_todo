...
function executeCLI(args: string[]): { stdout: string; stderr: string; exitCode: number } {
  try {
    const stdout = execSync(`node ${path.join(__dirname, '../../dist/index.js')} ${args.join(' ')}`, {
      encoding: 'utf8',
      stdio: 'pipe'
    });
...
function generateMalformedArguments(): string[][] {
  return [
    ...
    ['add', 'todo', '--flag1', '--flag2', '--flag3', '--flag4', '--flag5', '--flag6', '--flag7', '--flag8', '--flag9', '--flag10'],  // Too many flags
