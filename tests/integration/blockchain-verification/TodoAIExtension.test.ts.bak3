...
class TodoAIExtension {
      ...
      const insights = [
        {
          category: 'Priority',
          content:
            'This task is high priority based on its deadline and relationship to other tasks.',
        },
        {
          category: 'Dependency',
          content:
            'This task depends on 2 other tasks that need to be completed first.',
        },
        {
          category: 'Optimization',
          content:
            'Consider breaking this task down into smaller subtasks for better management.',
        },
      ];
   ...
   */
  async verifyAIGeneratedTodo(todoId: string): Promise<{
    authentic: boolean;
    verificationDetails: {
  ...
  describe('verifyAIGeneratedTodo', () => {
    ...
    it('should detect an inauthentic todo without blockchain verification', async () => {
      ...
      const todo = {
        ...
        description:
          'A todo that claims to be AI-generated but is not on blockchain',
        completed: false,
        priority: 'medium',
        tags: ['ai-generated'],
        created: new Date().toISOString(),
