...
jest.mock('../../apps/cli/src/services/ai/SecureCredentialManager', () => {
  return {
    SecureCredentialManager: jest.fn().mockImplementation(() => ({
      setCredential: jest.fn().mockResolvedValue({ id: 'test-id', providerName: 'test-provider' }),
...
jest.mock('fs', () => {
  ...
  return {
    ...
    existsSync: jest.fn().mockImplementation((path: string) => {
      if (path.includes('keyfile') || path.includes('keymetadata') || path.includes('audit')) {
        return true;
      }
    ...
    writeFileSync: jest
      .fn()
      .mockImplementation((path: string, data: Buffer | string, options?: unknown) => {
    ...
    readFileSync: jest.fn().mockImplementation((path: string, encoding?: string) => {
      ...
      if (path.includes('keymetadata')) {
        ...
        return encoding === 'utf8' ? JSON.stringify(metadata) : Buffer.from(JSON.stringify(metadata));
      }
      if (path.includes('secure_credentials')) {
    ...
    appendFileSync: jest
      .fn()
      .mockImplementation((path: string, data: string, options?: { mode?: number }) => {
        ...
        if (options) {
          mockFileContent.set(path + '_options', Buffer.from(JSON.stringify(options)));
        }
      }),
    mkdirSync: jest.fn(),
...
describe('Audit Log Security', () => {
  ...
  describe('Audit Log Content Security', () => {
    it('should log security-critical events', async () => {
      ...
      const credentialManager = {
        setCredential: jest.fn().mockImplementation(async (provider, credential, type) => {
        ...
        getCredential: jest.fn().mockImplementation(async (provider) => {
          if (provider === 'non-existent') {
            auditLogger.log('access_denied', { provider, reason: 'credential not found' });
            throw new Error('Credential not found');
          }
          auditLogger.log('credential_accessed', { provider });
          return 'test-credential';
        }),
        updatePermissions: jest.fn().mockImplementation(async (provider, level) => {
          auditLogger.log('permission_updated', { provider, newLevel: level });
          return true;
        }),
      ...
      expect(auditLogSpy).toHaveBeenCalledWith('credential_created', expect.any(Object));
      expect(auditLogSpy).toHaveBeenCalledWith('credential_accessed', expect.any(Object));
      expect(auditLogSpy).toHaveBeenCalledWith('permission_updated', expect.any(Object));
      expect(auditLogSpy).toHaveBeenCalledWith('access_denied', expect.any(Object));
    });
    ...
    it('should log AI operations with privacy-preserving details', async () => {
      ...
      expect(auditLogSpy).toHaveBeenCalledWith('ai_operation_summarize', expect.any(Object));
      expect(auditLogSpy).toHaveBeenCalledWith('ai_operation_categorize', expect.any(Object));
      expect(auditLogSpy).toHaveBeenCalledWith('ai_operation_analyze', expect.any(Object));
  ...
  describe('Blockchain Verification Audit', () => {
    it('should log blockchain verification events with tamper-evident properties', async () => {
      ...
      jest.spyOn(verificationService, 'createVerifiedSummary').mockImplementation(async (todos, summary, privacyLevel) => {
      ...
      expect(auditLogSpy).toHaveBeenCalledWith('blockchain_verification_created', expect.objectContaining({
  ...
  describe('Permission Changes Audit', () => {
    it('should log permission changes with before/after states', async () => {
      ...
      // Create a mock credential manager that logs audit events
      const credentialManager = {
        updatePermissions: jest.fn().mockImplementation(async (provider, newLevel) => {
      ...
      expect(auditLogSpy).toHaveBeenCalledWith('permission_updated', expect.objectContaining({
        provider: 'test-provider',
        previousLevel: AIPermissionLevel.STANDARD,
  ...
  describe('Failed Operation Auditing', () => {
    it('should log all failed security-critical operations', async () => {
      ...
      expect(auditLogSpy).toHaveBeenCalledTimes(3);
      expect(auditLogSpy).toHaveBeenCalledWith('credential_access_failed', expect.objectContaining({ success: false }));
      expect(auditLogSpy).toHaveBeenCalledWith('permission_update_failed', expect.objectContaining({ success: false }));
      expect(auditLogSpy).toHaveBeenCalledWith('ai_operation_failed', expect.objectContaining({ success: false }));
