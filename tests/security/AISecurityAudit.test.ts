...
describe('AI Security Audit', () => {
  ...
  describe('Credential Storage Security', () => {
    ...
    it('should prevent unauthorized credential access', async () => {
      ...
      await expect(
        secureCredentialManager.getCredential('unauthorized')
    ...
    it('should enforce credential expiration', async () => {
      ...
      const validCredential =
        await secureCredentialManager.getCredentialObject('valid');
      expect(validCredential.credentialValue).toBe('test-api-key');

      // Expired credential
      await expect(
        secureCredentialManager.getCredentialObject('expired')
  ...
  describe('Permissions and Access Control', () => {
    ...
    it('should enforce permission levels for AI operations', async () => {
      ...
      const mockVerificationService = new AIVerificationService(
    ...
    it('should enforce blockchain validation of credentials', async () => {
      ...
      await expect(
        secureCredentialManager.getCredential('xai')
      ).rejects.toThrow('Blockchain verification is no longer valid');
    });
    ...
    it('should prevent privilege escalation attempts', async () => {
      ...
      await expect(
        secureCredentialManager.updatePermissions(
          'xai',
  ...
  describe('Secure Communication Channels', () => {
    ...
    it('should prevent SSRF attacks in API requests', async () => {
      ...
      await expect(aiService.summarize([ssrfTodo])).rejects.toThrow(
    ...
    it('should detect and prevent request smuggling', async () => {
      ...
      await expect(aiService.summarize([smugglingTodo])).rejects.toThrow(
        'request smuggling'
      );
    });
  });
  ...
  describe('Data Privacy and Anonymization', () => {
    ...
    it('should handle data subject access requests', async () => {
      ...
      const mockVerifierAdapter = {
        listVerifications: jest.fn().mockImplementation(_userAddress => {
          // In production, would filter by the user
          return [mockVerificationRecord];
        }),
        getVerification: jest.fn().mockImplementation(_id => {
          return mockVerificationRecord;
        }),
      ...
      await expect(
        mockVerificationService.listVerifications('user-123')
  ...
  describe('Logging Security and PII Handling', () => {
    it('should redact sensitive information in logs', async () => {
      ...
      const sensitiveTodo = {
        ...
        description:
          'Update with SSN: 123-45-6789 and password: SecretPass123!',
        completed: false,
        createdAt: new Date().toISOString(),
        updatedAt: new Date().toISOString(),
      };
