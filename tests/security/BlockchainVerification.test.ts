...
describe('Blockchain Verification Security', () => {
  ...
  it('should enforce digital signatures for verification proofs', async () => {
    ...
    const mockBlockchainVerifier = {
      ...
      getSigner: jest.fn().mockReturnValue({
        getPublicKey: jest
          .fn()
          .mockReturnValue({ toBase64: jest.fn().mockReturnValue('test-key') }),
      }),
      generateProof: jest
        .fn()
        .mockResolvedValue(
          Buffer.from(JSON.stringify({ id: 'test-proof' })).toString('base64')
        ),
    };
  ...
  it('should protect against smart contract vulnerability exploits', async () => {
    ...
    const mockVerifierAdapter: MockSuiAIVerifierAdapter = {
      createVerification: jest.fn().mockImplementation(params => {
        ...
        const hasIntegerOverflow = requestStr.includes(
        ...
        const isTooLarge =
          requestStr.length > 10000 || responseStr.length > 10000;
  ...
  it('should handle different privacy levels securely', async () => {
    ...
    expect(
      (
        publicResult.verification as VerificationRecord & {
          requestData?: string;
        }
      ).requestData
    ).toBeDefined();
    ...
    expect(
      (hashResult.verification as VerificationRecord & { requestData?: string })
        .requestData
    ).toBeUndefined();
    ...
    expect(
      (
        privateResult.verification as VerificationRecord & {
  ...
  it('should enforce secure error handling for blockchain operations', async () => {
    ...
    const mockVerifierAdapter: MockSuiAIVerifierAdapter = {
      createVerification: jest.fn().mockImplementation(() => {
        ...
        (
          sensitiveError as Error & { details?: Record<string, unknown> }
        ).details = {
    ...
    expect(
      (testError as Error & { details?: unknown }).details
    ).toBeUndefined();
