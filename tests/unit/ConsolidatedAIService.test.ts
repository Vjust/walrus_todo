...
import { AIModelAdapter, AIProvider } from '../../src/types/adapters/AIModelAdapter';
...
jest.mock('../../src/services/ai/SecureCredentialService', () => ({
  secureCredentialService: {
    getCredential: jest.fn().mockResolvedValue({ key: 'mock-api-key', provider: 'xai' }),
    getAllCredentials: jest.fn().mockResolvedValue([{ key: 'mock-api-key', provider: 'xai' }]),
...
jest.mock('../../src/services/ai/AIProviderFactory', () => ({
  createDefaultAdapter: jest.fn().mockReturnValue({
    ...
    processWithPromptTemplate: jest.fn().mockResolvedValue({ result: 'Mock response' }),
  ...
  createFallbackAdapter: jest.fn().mockReturnValue({
    ...
    processWithPromptTemplate: jest.fn().mockResolvedValue({ result: 'Fallback response' }),
    completeStructured: jest.fn().mockResolvedValue({ result: {} })
  }),
  createProvider: jest.fn().mockResolvedValue({
    getProviderName: jest.fn().mockReturnValue('created-provider'),
    processWithPromptTemplate: jest.fn().mockResolvedValue({ result: 'Created response' }),
    completeStructured: jest.fn().mockImplementation(({ metadata }) => {
      switch (metadata.operation) {
        ...
        case 'suggest':
          return Promise.resolve({ result: ['Suggested todo 1', 'Suggested todo 2'] });
        case 'analyze':
          return Promise.resolve({ result: { themes: ['theme1'], bottlenecks: ['bottleneck1'] } });
        case 'group':
        ...
        case 'schedule':
          return Promise.resolve({ result: { todo1: { start: 0, duration: 2, due: 5 } } });
        case 'detect_dependencies':
          return Promise.resolve({ result: { dependencies: {}, blockers: {} } });
        case 'estimate_effort':
          return Promise.resolve({ result: { todo1: { effort: 3, reasoning: 'Moderate complexity' } } });
        default:
...
describe('Consolidated AIService', () => {
  ...
  it('should allow configuring service behavior', () => {
    const configManagerSpy = jest.spyOn(aiService['configManager'], 'updateGlobalConfig');
  ...
  describe('Core Operations', () => {
    ...
    it('should analyze todos', async () => {
      ...
      expect(result).toEqual({ themes: ['theme1'], bottlenecks: ['bottleneck1'] });
    });
  });
  ...
  describe('Advanced Operations', () => {
    ...
    it('should estimate effort', async () => {
      ...
      expect(result).toEqual({ todo1: { effort: 3, reasoning: 'Moderate complexity' } });
    });
  });
  ...
  describe('Verification Operations', () => {
    ...
    it('should categorize with verification', async () => {
      ...
      expect(mockVerificationService.createVerifiedCategorization).toHaveBeenCalled();
    ...
    it('should prioritize with verification', async () => {
      ...
      expect(mockVerificationService.createVerifiedPrioritization).toHaveBeenCalled();
    ...
    it('should suggest with verification', async () => {
      ...
      expect(mockVerificationService.createVerifiedSuggestion).toHaveBeenCalled();
  ...
  describe('Cache Operations', () => {
    ...
    it('should clear specific operation cache', () => {
      const clearOperationSpy = jest.spyOn(aiService['resultCache'], 'clearOperation');
