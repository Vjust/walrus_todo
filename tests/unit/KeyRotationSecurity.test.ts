...
describe('SecureCredentialManager Key Rotation and Security', () => {
            ...
            backupLocations: [
              {
                ...
                metadataBackupPath:
                  '/mock/home/.config/walrus-todo/key_backups/metadata_backup_2023-01-01.json',
              },
            ],
          });
  ...
  test('should properly validate credentials for expiration', async () => {
    ...
    (
      manager as unknown as {
        credentials: Record<string, typeof expiredCredential>;
      }
    ).credentials = {
    ...
    await expect(manager.getCredential('test-provider')).rejects.toThrow(
      'Credential for provider "test-provider" has expired'
    );
  ...
  test('should clean up old backups', () => {
    ...
    (
      manager as unknown as { backupCredentialsIfNeeded: () => void }
    ).backupCredentialsIfNeeded();
