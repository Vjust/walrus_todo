...
describe('SecureStorageService', () => {
  ...
  describe('encrypt', () => {
    it('should encrypt data successfully', async () => {
      ...
      mockCreateHash.mockReturnValue({
        ...
        digest: jest.fn().mockReturnValue(Buffer.from('hashed-key-32-bytes-long-enough'))
      });
      ...
      expect(result).toEqual({
        encrypted: Buffer.concat([mockEncrypted, Buffer.alloc(0)]).toString('base64'),
  ...
  describe('secure data management', () => {
    it('should handle secure store and retrieve operations', async () => {
      ...
      mockCreateHash.mockReturnValue({
        update: jest.fn().mockReturnThis(),
        digest: jest.fn().mockReturnValue(Buffer.from('hashed-secure-key-32-bytes-long'))
      });
    ...
    it('should handle large data encryption', async () => {
      ...
      mockCreateHash.mockReturnValue({
        update: jest.fn().mockReturnThis(),
        digest: jest.fn().mockReturnValue(Buffer.from('hashed-key-for-large-data-32byte'))
      });
