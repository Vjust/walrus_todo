...
describe('PermissionService', () => {
  ...
  describe('User Management', () => {
    ...
    it('should retrieve a user by ID', async () => {
      const createdUser = await permissionService.createUser(
        mockUsername,
        mockAddress
      );
  ...
  describe('Role Management', () => {
    it('should check if a user has a specific role', async () => {
      ...
      const hasCollaborator = await permissionService.hasRole(
        user.id,
        UserRole.COLLABORATOR
      );
      const hasAdmin = await permissionService.hasRole(user.id, UserRole.ADMIN);
      const hasSuperAdmin = await permissionService.hasRole(
        user.id,
    ...
    it('should remove a role from a user', async () => {
      const user = await permissionService.createUser(
        mockUsername,
        mockAddress,
        [UserRole.USER, UserRole.COLLABORATOR]
      );
  ...
  describe('Permission Checks', () => {
    ...
    it('should check direct permissions', async () => {
      ...
      const customPermission: Permission = {
        resource: createResourceIdentifier(
          ResourceType.SYSTEM,
          'custom-resource'
        ),
  ...
  describe('Owner Permissions', () => {
    ...
    it('should check if a user is a resource owner', async () => {
      ...
      const isNotOwner = await permissionService.isResourceOwner(
        user.id,
        ResourceType.TODO,
        'other-todo'
      );
  ...
  describe('Collaborator Permissions', () => {
    ...
    it('should grant custom actions to collaborators', async () => {
      ...
      const resource = createResourceIdentifier(ResourceType.LIST, listId);
      const canShare = await permissionService.hasPermission(
        user.id,
        resource,
        ActionType.SHARE
      );
