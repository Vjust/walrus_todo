...
export const createMockAIVerifierAdapter = (): jest.Mocked<AIVerifierAdapter> => {
  ...
  return {
    createVerification: jest.fn().mockImplementation(async (params: VerificationParams): Promise<VerificationRecord> => {
      return {
    ...
    getProviderInfo: jest.fn().mockImplementation(async (providerAddress: string): Promise<ProviderInfo> => {
      return {
    ...
    registerProvider: jest.fn().mockImplementation(async (params: ProviderRegistrationParams): Promise<string> => {
      return `provider_id_${params.name}`;
    }),
    getVerification: jest.fn().mockImplementation(async (verificationId: string): Promise<VerificationRecord> => {
      return {
        id: verificationId,
        requestHash: 'mock_request_hash',
        responseHash: 'mock_response_hash',
...
export const createMockBlockchainVerifier = () => {
  ...
  return {
    verifyOperation: jest.fn().mockImplementation(async (params: VerificationParams): Promise<VerificationRecord> => {
      return {
        id: `verification_${Date.now()}`,
        requestHash: 'mock_request_hash',
        responseHash: 'mock_response_hash',
    ...
    generateVerificationProof: jest.fn().mockResolvedValue('base64_encoded_proof'),
