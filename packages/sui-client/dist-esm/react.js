...
import { createNetworkConfig, SuiClientProvider, WalletProvider, useCurrentAccount as useCurrentAccountDApp, useConnectWallet, useDisconnectWallet, useSignAndExecuteTransaction, ConnectModal, useWallets, useSuiClient, } from '@mysten/dapp-kit';
import { getFullnodeUrl } from '@mysten/sui/client';
import { QueryClient, QueryClientProvider } from '@tanstack/react-query';
import { Transaction } from '@mysten/sui/transactions';
import { bcs } from '@mysten/sui/bcs';
import { normalizeTransactionResult, normalizeOwnedObjectsResponse, checkVersionCompatibility } from './compatibility';
import { SuiClientError, WalletNotConnectedError, TransactionError, } from './types';
import { loadAppConfig, clearConfigCache, } from './config';
...
// Create the context
const WalTodoWalletContext = createContext(null);
/**
 * Hook to use the WalTodo wallet context
 */
export function useWalTodoWallet() {
 ...
 */
function WalTodoWalletContextProvider({ children }) {
    ...
    const signAndExecuteTransaction = useCallback(async (txb) => {
        try {
            setError(null);
            ...
            const result = await signAndExecute({
                transaction: txb // Type assertion to handle version compatibility
            });
    ...
    const createTodoNFT = useCallback(async (params) => {
        if (!config || !account) {
            throw new WalletNotConnectedError();
        }
    ...
    const updateTodoNFT = useCallback(async (params) => {
        if (!config || !account) {
            throw new WalletNotConnectedError();
        }
        const packageId = config.contracts.todoNft.packageId;
    ...
    const completeTodoNFT = useCallback(async (objectId) => {
        if (!config || !account) {
            throw new WalletNotConnectedError();
        }
        const packageId = config.contracts.todoNft.packageId;
    ...
    const deleteTodoNFT = useCallback(async (objectId) => {
        if (!config || !account) {
            throw new WalletNotConnectedError();
        }
        const packageId = config.contracts.todoNft.packageId;
    ...
    const getTodosFromBlockchain = useCallback(async () => {
        if (!config || !account) {
            throw new WalletNotConnectedError();
        }
        try {
    ...
    return (_jsxs(WalTodoWalletContext.Provider, { value: contextValue, children: [children, _jsx(ConnectModal, { trigger: _jsx("div", { style: { display: 'none' } }), open: isModalOpen, onOpenChange: (open) => {
                    setIsModalOpen(open);
...
export function WalTodoWalletProvider({ children, queryClient = defaultQueryClient, defaultNetwork = 'testnet', autoConnect = true, }) {
...
export function useTodoNFTOperations() {
    const { createTodoNFT, updateTodoNFT, completeTodoNFT, deleteTodoNFT, getTodosFromBlockchain, } = useWalTodoWallet();
    return {
        createTodoNFT,
        updateTodoNFT,
        completeTodoNFT,
...
// Re-export types
export * from './types';
export { loadAppConfig, getNetworkConfig, getCachedConfig, clearConfigCache, isConfigurationComplete, } from './config';
