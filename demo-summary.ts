...
async function showTodoForAJ() {
  try {
    ...
    process.stdout.write(
      '1. Configure Walrus: Create ~/.config/walrus/client_config.yaml\n'
    );
    process.stdout.write(
      '2. Get WAL tokens: walrus --context testnet get-wal\n'
    );
    process.stdout.write('3. Store the todo: walrus store <todo-file>\n');
    process.stdout.write(
      '4. Create NFT with the blob ID for Sui scanner visibility\n'
    );
  } catch (error) {
