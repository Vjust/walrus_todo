...
export function useSuiTodos(): UseSuiTodosReturn {
  ...
  const checkHealth = useCallback(async () => {
    try {
      ...
      setError(`Health check failed: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }, [setError]);
  ...
  const switchToNetwork = useCallback(async (network: NetworkType) => {
    setLoading(true);
    setError(null);
  ...
  const createTodo = useCallback(async (params: CreateTodoParams): Promise<TransactionResult> => {
    setLoading(true);
    setError(null);

    try {
      // First, create the todo locally
      const newTodo = addTodo('default', {
        title: params.title,
      ...
      if (isWalletReady && address && walletContext.signAndExecuteTransaction) {
        ...
        const todoPromise = storeTodoOnBlockchain('default', newTodo.id, walletSigner, address).then(
          objectId => ({ digest: objectId || undefined })
        );
  ...
  }, [isWalletReady, address, trackTransaction, refreshTodos, setError, setLoading, walletContext.signAndExecuteTransaction]);

  // Update todo on blockchain
  const updateTodo = useCallback(async (params: UpdateTodoParams): Promise<TransactionResult> => {
  ...
  // Complete todo
  const completeTodo = useCallback(async (todoId: string): Promise<TransactionResult> => {
    setLoading(true);
    setError(null);

    try {
      // Find the todo
      const todo = state.todos.find(t => t.id === todoId || t.objectId === todoId);
      if (!todo) {
      ...
      if (todo.blockchainStored && todo.objectId && isWalletReady && address && walletContext.signAndExecuteTransaction) {
        const walletSigner: WalletSigner = {
          signAndExecuteTransaction: walletContext.signAndExecuteTransaction,
          address
        };

        const completePromise = completeTodoOnBlockchain('default', todo.id, walletSigner, address).then(
          success => ({ digest: success ? 'completed' : undefined })
        );
        
        const result = await trackTransaction(completePromise, 'Complete Todo NFT');
  ...
  }, [state.todos, isWalletReady, address, trackTransaction, refreshTodos, setError, setLoading, walletContext.signAndExecuteTransaction]);

  // Transfer todo NFT (delete locally after transfer)
  const deleteTodo = useCallback(async (todoId: string): Promise<TransactionResult> => {
    setLoading(true);
    setError(null);
    ...
    try {
      ...
      if (!todo.blockchainStored) {
        // Delete local todo
        const deleted = deleteLocalTodo('default', todo.id, address || undefined);
        if (deleted) {
          await refreshTodos();
        ...
        throw new Error('Blockchain todos cannot be deleted, only transferred to another address');
      }
