...
export function BasicEventSubscription() {
  ...
  const { isConnected, isConnecting, error, startSubscription, stopSubscription } = useBlockchainEvents({
    autoStart: true,
  ...
  return (
    <div className="p-4 border rounded-lg">
      ...
      <div className="grid grid-cols-2 gap-4">
        ...
        <div>
          <h4 className="font-medium mb-2">Recent Events ({recentEvents.length})</h4>
          <div className="space-y-1 max-h-40 overflow-y-auto">
            {recentEvents.slice(0, 5).map((event, index) => (
              <div key={index} className="text-xs p-2 bg-blue-50 rounded">
                <span className="font-mono">{event.type}</span>: {JSON.stringify(event.data, null, 2).substring(0, 50)}...
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="mt-4">
        ...
        <span className="ml-2 text-sm text-gray-600">
          {isConnected ? 'Receiving live updates' : 'Connect wallet for live updates'}
        </span>
      </div>
    </div>
  );
    ...
    <div className="p-4 border rounded-lg">
      ...
      <div className="mb-4">
        ...
        <span className="ml-4 text-sm text-gray-600">
          Total todos: {syncedTodos.length} | Connected: {isConnected ? '✅' : '❌'}
        </span>
      </div>
          ...
          <div key={todo.id} className="flex items-center justify-between p-2 bg-gray-50 rounded">
            <div>
              ...
              {todo.blockchainStored && (
                <span className="ml-2 px-2 py-1 text-xs bg-blue-100 text-blue-800 rounded">On-chain</span>
              )}
            </div>
...
export function ConnectionStatusManagement() {
  ...
  return (
    <div className="p-4 border rounded-lg">
      <h3 className="text-lg font-semibold mb-4">Connection Status Management</h3>
...
export function CompleteRealtimeTodoApp() {
  ...
  const handleTodoComplete = async (todo: Todo) => {
    ...
    setTodos(prev => prev.map(t => 
      t.id === todo.id ? { ...t, completed: true, completedAt: Date.now() } : t
    ));
  };
  ...
  if (!connected) {
    return (
      <div className="p-4 border rounded-lg text-center">
        <h3 className="text-lg font-semibold mb-4">Complete Real-time Todo App</h3>
        <p className="text-gray-600 mb-4">Please connect your wallet to use the real-time todo application.</p>
      </div>
    );
  }
...
export default function BlockchainEventsDemo() {
  ...
  const examples = {
    basic: { component: BasicEventSubscription, title: 'Basic Event Subscription' },
    handlers: { component: TodoEventHandlers, title: 'Todo Event Handlers' },
    sync: { component: TodoStateSync, title: 'Todo State Sync' },
    status: { component: ConnectionStatusManagement, title: 'Connection Status' },
    complete: { component: CompleteRealtimeTodoApp, title: 'Complete Todo App' },
  };
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <div className="mb-8">
        <h1 className="text-3xl font-bold mb-4">Blockchain Events Usage Examples</h1>
        <p className="text-gray-600 mb-6">
          Interactive examples demonstrating real-time blockchain event integration for TodoNFT smart contracts.
        </p>
      ...
      <div className="mt-8 p-4 bg-yellow-50 border-l-4 border-yellow-400">
        ...
        <ul className="mt-2 text-sm text-yellow-700 space-y-1">
          ...
          <li>• Some features require actual blockchain transactions to trigger events</li>
        </ul>
      </div>
    </div>
  );
