...
export default function TodoList({ listName }: TodoListProps) {
  ...
  const mergedTodos = [...todos, ...blockchainTodos].reduce((acc: Todo[], todo) => {
    // Remove duplicates based on objectId (blockchain todos take precedence)
    const existing = acc.find(t => t.objectId && t.objectId === todo.objectId);
    if (!existing) {
      acc.push(todo);
  ...
  const toggleTodoCompletion = async (id: string) => {
    ...
    const updatedTodos = displayTodos.map(todoItem => 
      todoItem.id === id ? { ...todoItem, completed: !todoItem.completed, completedAt: !todoItem.completed ? Date.now() : undefined } : todoItem
    )
    setTodos(updatedTodos)
    ...
    try {
      if (todo.blockchainStored && todo.objectId && signAndExecuteTransaction) {
        ...
        const result = await completeTodoOnBlockchain(todo.objectId, signAndExecuteTransaction, address || '');
        ...
        if (result.success) {
          ...
          throw new Error(result.error || 'Failed to complete todo on blockchain');
        }
      } else {
        // Update in wallet-specific local storage
        const updatedTodo = { ...todo, completed: !todo.completed, completedAt: !todo.completed ? Date.now() : undefined }
        ...
        {connected && (
          <div className="mt-4">
            <div className="flex items-center space-x-2">
              ...
              <span className="text-xs text-ocean-medium/70">Blockchain Connected</span>
            </div>
  ...
  const handleStoreOnBlockchain = async (todo: Todo) => {
    ...
    try {
      ...
      alert('Feature coming soon: Store existing todo as NFT on Sui blockchain');
    } catch (error) {
  ...
  const handleDeleteTodo = async (todo: Todo) => {
    ...
    try {
      if (todo.blockchainStored && todo.objectId && signAndExecuteTransaction) {
        ...
        const result = await deleteTodoOnBlockchain(todo.objectId, signAndExecuteTransaction, address || '');
        ...
        if (result.success) {
          ...
          throw new Error(result.error || 'Failed to delete todo from blockchain');
        }
      } else {
