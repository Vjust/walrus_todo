...
export default function CreateTodoForm({ listName, onTodoAdded }: CreateTodoFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (createOnBlockchain && connected && address && signAndExecuteTransaction) {
        ...
        try {
          ...
          if (blockchainResult.success && newTodo && blockchainResult.objectId) {
          ...
          console.warn('Blockchain creation failed, but local todo was created:', blockchainError);
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
        ...
        {connected && (
          <div className="flex items-center">
            <input
              ...
              className="w-4 h-4 rounded text-ocean-medium focus:ring-ocean-light"
            />
            <label htmlFor="createOnBlockchain" className="ml-2 text-sm text-ocean-medium dark:text-ocean-light">
      ...
      <div className="flex justify-between items-center pt-2">
        ...
        <button
          type="submit"
          disabled={isSubmitting || !title.trim() || (createOnBlockchain && !connected)}
          className={`ocean-button ${
            (isSubmitting || !title.trim()) ? 'opacity-70 cursor-not-allowed' : ''
          }`}
        >
          {isSubmitting 
            ? (isCreatingOnChain ? 'Creating NFT...' : 'Adding...') 
